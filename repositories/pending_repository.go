...
type PendingActionRepository interface {
	...
	Claim(ctx context.Context, userID int, token string, now time.Time) (*models.PendingAction, error)
...
func (r *gormPendingActionRepository) Create(ctx context.Context, action *models.PendingAction) error {
...
func (r *gormPendingActionRepository) Claim(ctx context.Context, userID int, token string, now time.Time) (*models.PendingAction, error) {
	result := r.db.WithContext(ctx).Model(&models.PendingAction{}).
	...
	if err := r.db.WithContext(ctx).Where("token = ?", token).First(&action).Error; err != nil {
...
func (r *gormPendingActionRepository) DeleteExpired(ctx context.Context, before time.Time) (int64, error) {
	result := r.db.WithContext(ctx).Where("expires_at < ?", before).Delete(&models.PendingAction{})
	if result.Error != nil {
